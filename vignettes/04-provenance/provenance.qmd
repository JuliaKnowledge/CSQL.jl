---
title: "Provenance and Local Causal Models"
author: "Simon Frost"
subtitle: "Tracking evidence sources behind causal claims"
format:
  html:
    code-fold: false
    toc: true
...
A key feature of Csql is **provenance tracking**: every causal claim is linked back to its source document and local causal model (LCM). This enables:
...
This vignette covers the `LocalCausalModel` type, the `atlas_edge_support` table, and provenance queries.

## Local Causal Models

A *Local Causal Model* (LCM) is a set of causal triples extracted from a single document or hypothesis. Instead of adding triples one by one, you can package them as an LCM:
...
The `atlas_edge_support` table tracks which documents and LCMs support each edge.
...
When multiple LCMs assert the same causal relationship, CSQL.jl aggregates them. Let's find these overlaps:
...
Next: [Atlas Merging](../05-merging/merging.html) covers combining causal databases from different research domains.
